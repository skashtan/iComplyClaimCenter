...
Ext.define('gw.AccordionPanel', {
        ...
        if (Array.isArray(me.items)) {
            for (var i = 0; i < me.items.length; i++) {
                ...
                if (typeof item.shortcut == "string") {
                    var shortcutConfig = me.getKeyMapConfig(item.shortcut, me, Ext.getDoc(), gw.app.handleAction.bind(null, null, item.eventId.slice(), {}, {}));
                    var keyMap = me.addKeyMap(item.shortcut, Ext.getDoc(), shortcutConfig, me);
    ...
    onAfterRender: function() {
        if (this.header && this.header.el) {
            // @SenchaUpgrade: making the header selectable by calling a private extjs function. This header is correspond
            // to the title in left navigation.
            this.header.el.selectable();
        }
    },
