...
Ext.define('gw.app', function () {
    ...
    function getDataChanges() {
        ...
        /**
         *  Code was added to skip the call to collectChangesFromRecord call when
         *    (store.type === 'tree' && rs[i].modified.parentId === null)
         *  (hash commit #4167eeb: "Ignore tree with modified parentId that is null. Fixes row offset split problem.")
         *
         *  Unfortunately, the behavior associated with "Fixes row offset split problem" is lost to history.
         *  More problematic is skipping on the above condition causes checking the checkbox of a just expanded
         *  child in a rowtree to be lost on form submit.
         *
         *  Reverting the change but leaving this comment as a bread crumb in case we figure out/encounter the issue
         *  it was first added to deal with.
        */
    ...
    function _requestCallback(requestOptions, success, response) {
        ...
        _restoreScrollPositions(requestOptions);
    ...
    /**
     * Checks requestOptions to see if scrollPositions is set.  If set, applies the scroll positions.
     * @param requestOptions
     *
     * @UpgradedSencha 5.1 Use scroller object
     */
    function _restoreScrollPositions(requestOptions) {
...
//          {key:Ext.event.Event.ENTER, shift:false, alt:false, fn:_handleEnterKey} // ENTER key
    ...
    return {
        ...
        /**
         * Returns an array of keyCodes, one keycode for each letter in the string.
         ...
         * @params params
         */
        handleAction: function (evt, htmlElem, options, params, comp) {
        ...
        handleCompAction: function (comp, evt, el) {
            ...
            function doIt(param) {
                ...
                if (comp.postOnChangeTarget) {
                    var compId = gw.app.getPocId(comp, el && el.dom || comp.getEl().dom);
                    gw.app.requestViewRoot(compId, {updateData:true, postOnChangeTarget:comp.postOnChangeTarget});
                } else {
                    gw.app.handleAction(evt, el && el.dom || comp.getEl().dom, options, undefined, comp);
                }
            }
            ...
            function applyChanges() {
                ...
                if (isPostOnChange) {
                    ...
                    if (targetId != null) {
                        ...
                        var rootId = field.eventParam || field.eventId || field.name || field.checkboxName || field.id;
                        gw.app.requestViewRoot(rootId, params, Ext.fly(targetId),
                            (callback) ? {
                        ...
                        var eventParam = field.eventParam || field.eventId || field.name || field.hiddenName || field.id;
                        _callServer('_refresh_', eventParam, extraParams, true);
                    }
        ...
        requestDataUpdate: function(dataRequesterId, eventSource, eventParam) {
            var requestParams = {};
            var ajaxInfo = {dataRequesterId: dataRequesterId};
            requestParams.ajaxRequestInfo = Ext.JSON.encode([ajaxInfo]);
            requestParams.skipFocusOnPOC = true;
            _callServer(eventSource, eventParam, requestParams, true);
        },
