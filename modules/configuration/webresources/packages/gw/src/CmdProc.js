...
Ext.define('gw.CmdProc', {
     ...
     * @param cmdInfo
     */
    replace : function (cmdInfo, response, requestOptions) {
        var comp = Ext.ComponentManager.get(cmdInfo.target);
        if (comp == null) {
        ...
        if (!cmdInfo.items) {
            ...
            // SK - 30Jan2018: Previously, this remove/insert were contained within a deferred function
            // call with a note stating that it need to be pushed to another render cycle in order for
            // GW charts to render correctly.  The defer caused side effects - e.g. causing issues when
            // restoring scroll position.  Removal of the defer does not seem to cause any problems with
            // chart rendering.
    ...
    updateValues : function(cmdInfo, response) {
        ...
        for (var i = 0; i < cmdInfo.items.length;i++) {
            ...
            if (item.value !== undefined) {
                // ExtJS4 upgrade: do not fire change events when value is set from the server.
