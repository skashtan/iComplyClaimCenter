Ext.define('gw.override.Component', {
    ...
    beforeRender: function () {
        ...
        if (!me.autoGenId && // @SenchaUpgrade Use a private ExtJS API to detect if this is an auto-generated overflow menuitem. Do not create shortcut for overflow menu to avoid conflict with the original button.
            ...
            if (shortcut) {
                ...
                me.scKeyMapConfig = me.getKeyMapConfig(me.shortcut, me, null, me.onShortcutActivated);
                me.showShortcutInButtonText(me.scKeyMapConfig.shift, me.scKeyMapConfig.key);
            }

            if (me.isDefault) {
                //@UpgradedSencha we let the app know what is the default key id. This will be accessible from any component.
                gw.app.defaultShortcutId = me.eventId;
                //--------
    ...
    getKeyMapConfig: function(kbShortcut, shortcutScope, shortcutTarget, shortcutFunction) {
        var shortcut = kbShortcut.toLowerCase();
        var matches = shortcut.match(/(shift)*(.+)/i); // checks for "shift" prefix
        var bShift = matches[1] ? true : false;
        var keyCodes = gw.app.stringToKeyCodes(matches[2]);

        var numPadKeyCodes = this.getNumpadKeyCodes(keyCodes);
        if (numPadKeyCodes.length > 0) {
            keyCodes = keyCodes.concat(numPadKeyCodes);
        }
        ...
        if (shortcutTarget) {
            keyMapConfig.target = shortcutTarget;
        }

        return keyMapConfig;
    },

    getNumpadKeyCodes: function(nonNumpadKeyCodes) {
        var numpadKeyCodes = [];
        var KEYCODE_0 = 48, KEYCODE_9 = 57;
        var currentKeyCode;

        for (var i = 0; i < nonNumpadKeyCodes.length; i++) {
            currentKeyCode = nonNumpadKeyCodes[i];
            if (KEYCODE_0 <= currentKeyCode && currentKeyCode <= KEYCODE_9) {
                // Keycodes 48 to 57 correspond to the digits 0 to 9 in sequential order.  Numbers entered via a number pad may use keycode
                // values 96 to 105; add the corresponding numpad keycode shortcut if we bump into a number shortcut.
                numpadKeyCodes.push(parseInt(currentKeyCode) + 48);
            }
        }
    ...
    addKeyMap: function (key, scopeEl, config, comp) {
        ...
        if (mapsPerKey.length > 0) {
            ...
            oldComp = old.comp;
            // origText is only set if there is a shortcut defined. If origText is not defined then
            // there is no shortcut; which mean no need to change the text
    ...
    showShortcutInButtonText: function (bShift, key) {
        ...
        if (text) {
            ...
            if (Ext.isArray(key) && key.length > 0) {
                key = key[0] === 190 ? '>' :
